...
export default async function Job() {
  let jobs: JobType[] = await sanityFetch({
    query: jobQuery,
    tags: ["job"],
  });

  // Sort jobs so most recent appears first
  jobs = jobs.sort((a, b) => {
        ...
        <Slide delay={0.18}>
          <div className="grid grid-cols-1 gap-y-10">
            {jobs.map((job) => (
              <div
                key={job._id}
                className="flex items-start gap-x-4 max-w-2xl relative
                before:absolute before:top-[5rem] before:left-9 before:w-[1px] 
                before:h-[calc(100%-70px)] dark:before:bg-zinc-800 before:bg-zinc-200
                last:before:hidden"
              >
                <RefLink
                  href={job.url}
